...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (doctorDto) => {
    try {
      const data = await ApiService.registrarOdontologo(doctorDto);
      setUser(data.usuario);
      setToken(data.token);
      setIsAuthenticated(true);
      localStorage.setItem(STORAGE_KEY_USER, JSON.stringify(data.usuario));
  ...
  return (
    <AuthContext.Provider value={{ user, token, isAuthenticated, login, register, logout, loginDirecto }}>
      {children}
